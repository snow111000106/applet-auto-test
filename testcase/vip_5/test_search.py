...
class SearchPageTest(BaseCase):
    ...
    def test_authorLib_filter_by_one_second_label(self):
        """
        测试个人版用户的博主库无法筛选单个博主二级标签
        """
        self.searchpage.select_tab(tab_name='博主')
        self.searchpage.authorLib_filter_by_label(first_label='摄影', second_label='摄影器材')
        status = self.searchpage.is_vip_have_authority(tab_name='blogger')
        self.assertTrue(status)

    def test_authorLib_filter_by_multi_first_label(self):
        """
        测试个人版用户的博主库无法筛选多个博主一级标签
        """
        self.searchpage.select_tab(tab_name='博主')
        first_label = ['游戏', '运动健身']
    ...
    def test_authorLib_filter_by_one_fans_label(self):
        """
        测试个人版用户的博主库无法筛选单个粉丝标签
        """
        self.searchpage.select_tab(tab_name='博主')
        self.searchpage.authorLib_filter_by_fans_label(fans_labels='爱美少女')
        status = self.searchpage.is_vip_have_authority(tab_name='blogger')
        self.assertTrue(status)

    def test_authorLib_filter_by_multi_fans_label(self):
        """
        测试个人版用户的博主库无法筛选多个粉丝标签
        """
        self.searchpage.select_tab(tab_name='博主')
    ...
    def test_authorLib_filter_by_fans_level(self):
        """
        测试个人版用户的博主库无法以筛选粉丝量级
        """
        self.searchpage.select_tab(tab_name='博主')
        self.searchpage.select_author_filter(filter_types='粉丝信息', types='粉丝量级', value='1000-1W')
        status = self.searchpage.is_vip_have_authority(tab_name='blogger')
        self.assertTrue(status)
    ...
    def test_authorLib_filter_by_fans_area(self):
        """
        测试个人版用户的博主库无法以筛选粉丝地区
        """
        self.searchpage.select_tab(tab_name='博主')
        self.searchpage.select_author_filter(filter_types='粉丝信息', types='粉丝地区', value=['江苏省', '苏州市'])
        status = self.searchpage.is_vip_have_authority(tab_name='blogger')
        self.assertTrue(status)

    def test_authorLib_filter_by_fans_age(self):
        """
        测试个人版用户的博主库无法以筛选粉丝年龄
        """
        self.searchpage.select_tab(tab_name='博主')
        self.searchpage.select_author_filter(filter_types='粉丝信息', types='粉丝年龄', value='35-44')
        status = self.searchpage.is_vip_have_authority(tab_name='blogger')
        self.assertTrue(status)

    def test_authorLib_filter_by_liked(self):
        """
        测试个人版用户的博主库无法以筛选赞藏总数
        """
        self.searchpage.select_tab(tab_name='博主')
        self.searchpage.select_author_filter(filter_types='内容筛选', types='赞藏总数', value='1W-5W')
        status = self.searchpage.is_vip_have_authority(tab_name='blogger')
        self.assertTrue(status)
    ...
    def test_noteLib_filter_by_interaction(self):
        ...
        """
        self.searchpage.select_tab(tab_name='笔记')
        self.searchpage.select_note_filter(filter_types='笔记筛选', types='互动量', value='1W-3W')
        time.sleep(2)
        try:
            info = self.searchpage.get_note_info(num=10)
            for i in range(len(info)):
                self.assertGreaterEqual(info[i][4], 10000)
                self.assertLessEqual(info[i][4], 30000)
        finally:
            self.capture('result')
    ...
    def test_noteLib_filter_by_liked(self):
        ...
        """
        self.searchpage.select_tab(tab_name='笔记')
        self.searchpage.select_note_filter(filter_types='笔记筛选', types='点赞数', value='500-1000')
        time.sleep(2)
        try:
            info = self.searchpage.get_note_info(num=10)
            for i in range(len(info)):
                self.assertGreaterEqual(info[i][5], 500)
                self.assertLessEqual(info[i][5], 1000)
        finally:
            self.capture('result')
    ...
    def test_noteLib_filter_by_fans_area(self):
        ...
        """
        self.searchpage.select_tab(tab_name='笔记')
        self.searchpage.select_note_filter(filter_types='粉丝信息', types='粉丝地区', value= ['山西省', '太原市'])
        time.sleep(2)
        info = self.searchpage.get_note_info(num=10)
        self.assertEqual(len(info), 10)
    ...
    def test_brandLib_filter_by_first_label(self):
        """
        测试个人版用户的品牌库可筛选一级品牌标签
        """
        self.searchpage.select_tab(tab_name='品牌')
        self.searchpage.brandLib_filter_by_label(first_label='汽车/工具', second_label='全部')
    ...
    def test_brandLib_filter_by_second_label(self):
        """
        测试个人版用户的品牌库可筛选二级品牌标签
        """
        self.searchpage.select_tab(tab_name='品牌')
        self.searchpage.brandLib_filter_by_label(first_label='互联网', second_label='电子商务')
        label = self.searchpage.get_brand_label(num=10)
        for i in range(len(label)):
            self.assertIn('电子商务', label[i][2])
    ...
    def test_brandLib_filter_by_interaction(self):
        ...
        try:
            info = self.searchpage.get_brand_info(num=10)
            for i in range(len(info)):
                self.assertGreaterEqual(info[i][3], 10000)
                self.assertLessEqual(info[i][3], 30000)
        finally:
            self.capture('result')
    ...
    def test_brandLib_filter_by_fans_area(self):
        ...
        """
        self.searchpage.select_tab(tab_name='品牌')
        self.searchpage.select_brand_filter(filter_types='粉丝信息', types='粉丝地区', value=['广东省', '广州市'])
        time.sleep(2)
        info = self.searchpage.get_brand_info(num=10)
        self.assertEqual(len(info), 10)
    ...
    def test_kindLib_filter_by_other_labels(self):
        ...
        """
        self.searchpage.select_tab(tab_name='品类')
        self.searchpage.kindLib_filter_by_other_labels(types='日用/宠物')
        label = self.searchpage.get_kind_label(num=10)
        for i in range(len(label)):
            self.assertIn('日用/宠物', label[i][2])
    ...
    def test_hotWordLib_filter_by_second_label(self):
        """
        测试个人版用户的热搜词库可筛选单个二级标签
        """
        self.searchpage.select_tab(tab_name='热搜词')
        self.searchpage.hotWordLib_filter_by_label(first_label='宠物', second_label='狗')
        time.sleep(2)
        info = self.searchpage.get_hot_word_info(num=10)
        self.assertEqual(len(info), 10)
    ...
    def test_hotWordLib_filter_by_other_label(self):
        ...
        """
        self.searchpage.select_tab(tab_name='热搜词')
        self.searchpage.hotWordLib_filter_by_other_labels(types='减肥')
        time.sleep(2)
        info = self.searchpage.get_hot_word_info(num=10)
        self.assertEqual(len(info), 10)
