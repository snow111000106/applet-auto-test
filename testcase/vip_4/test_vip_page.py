...
class VipPageTest(BaseCase):
    ...
    def test_user_info(self):
        ...
        self.assertEqual(vip_name, Base.get_vip_info(4)['vip_name'])
        self.assertEqual(vip_time, Base.get_vip_info(4)['vip_time'])
        self.assertEqual(name, Base.get_vip_info(4)['mobile'])
    ...
    @minium.ddt_case((1, '专业版12个月', '16999'), (2, '专业版3个月', '5699'),
                     (3, '专业版1个月', '2199'), (4, '专业版24个月', '27999'))
    def test_pop_info(self, value):
        """"
        测试购买会员弹窗的信息是否显示正确
        """
        card_id, re_pre, re_price = value
        self.vippage.choose_vip_version(version='专业版', card_id=card_id)
        time.sleep(1)
        self.vippage.click_ele(types='vip_btn')
        time.sleep(2)
    ...
    @minium.ddt_case(('企业版', '立即升级'), ('专业版', '立即续费'), ('个人版', '立即开通'))
    def test_vip_btn_info(self, value):
        """
        测试专业版用户查看vip续费按钮，文案显示正确
        """
        types, re_text = value
    ...
    @minium.ddt_case((1, ['不使用优惠券', '818特惠 立减800元']), (2, ['不使用优惠券', '818特惠 立减250元']),
                     (3, ['不使用优惠券', '818特惠 立减100元']))
    ...
    @minium.skipUnless(condition=config.is_activity is True, reason="活动期间有优惠券入口")
    @minium.ddt_case((1, '818特惠 立减800元'), (2, '818特惠 立减250元'),
                     (3, '818特惠 立减100元'))
